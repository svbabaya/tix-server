...
```
## Настраиваем доступ к Axis из macOS
После настройки моста для доступа из Lima к камере может появиться проблема доступа к Axis из macOS. Такое возможно если macOS сохранила в памяти неверный физический адрес камеры. Принудительная связка IP + MAC-адрес + интерфейс en8 через arp -s и route «пробьет» путь в обход виртуального моста Lima:
- Добавляем статическую ARP-запись для камеры:
```
sudo arp -s 169.254.211.120 00:40:8c:e8:68:71
```
- Посылаем ping 408:
```
ping -s 408 169.254.211.120
```
Команда ping -s 408 — это специальный пинг, который камеры Axis распознают как команду принять предложенный IP-адрес из статической ARP-записи.
-  Подождать 10 секунд:
```
sleep 10
```
- Обычный ping для проверки:
```
ping 169.254.211.120
```
Чтобы настройка сохранилась, установим IP в админке камеры:
- Перехолим в System Options → Network → TCP/IP
- Вместо Obtain IP address via DHCP выберите Use the following IP address
Устанавливаем:
- IP Address: 169.254.211.120
- Subnet mask: 255.255.0.0
- Default router: оставьте пустым
Сохраняем.
