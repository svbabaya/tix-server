## Как собрать библиотеку opencv для камеры Axis с помощью кросскомпилятора

# 1. Скачать opencv нужной версии и разархивировать или клонировать с github. На старых камерах с последними версиями opencv могут быть проблемы. Для камер mips рекомендуется использовать opencv не выше 3 версии.

# 2. В корневой папке opencv создать файл конфигурации для cmake с учетом архитектуры камеры: mips.toolchain.cmake. Камеры mips не имеют аппаратной поддержки вычислений с плавающей точкой, поэтому при компиляции нужно включать режим soft-float

# 3. Конфигурация в mips.toolchain.cmake, необходимо учитывать, какие библиотеки нужны (статические/динамические) наличие VFP/FPU в существующей архитектуре и прочие особенности архитектуры:
...
# 5. Зайти в build из терминала Linux и создать сонфигурацию для сборки:
