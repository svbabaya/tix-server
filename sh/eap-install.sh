...
do_get_syslog() {
	...
	$CURL --anyauth -s -S -u root:$password "http://${axis_device_ip}/axis-cgi/systemlog.cgi" >.${axis_device_ip}-var_log_messages.txt 2>&1
	curlres=$?
...
if [ -z "$1" ]; then
	...
	echo -n "Target root password [$password]: "
	...
	if [ -z "$axis_device_ip" ] || [ -z "$password" ] ; then
		echo -n "Target IP address [$axis_device_ip]: "
		axis_device_ip=$(getInput $axis_device_ip)
		echo -n "Target root password [$password]: "
		password=$(getInput $password)
...
for myeap in *.eap ; do
	...
	case "$action" in
		install)
			...
			$CURL --anyauth -s -S -F packfil=@${myeap} -u root:${password} "http://${axis_device_ip}/axis-cgi/admin/applications/upload.cgi?reload_page=yes" 2>&1 >/tmp/.curlmsg$$
		...
		start)
			...
			$CURL --anyauth -s -S -u root:${password} "http://${axis_device_ip}/axis-cgi/applications/control.cgi?action=start&package=${mydir}&reload_page=yes" 2>&1 >/tmp/.curlmsg$$
		...
		stop)
			echo "${action}ing $mydir"
			$CURL --anyauth -s -S -u root:${password} "http://${axis_device_ip}/axis-cgi/applications/control.cgi?action=stop&package=${mydir}&reload_page=yes" 2>&1 >/tmp/.curlmsg$$
			...
			if [ $grepokmsg -eq 0 ]; then
				...
				if [ $stopres -ne 0 ]; then
					echo "Failed ${action}ing $mydir stopres=$stopres"
					exit $stopres
				fi
			fi
			;;
		remove)
			echo "removing $mydir"
			$CURL --anyauth -s -S -u root:${password} "http://${axis_device_ip}/axis-cgi/applications/control.cgi?action=remove&package=${mydir}&reload_page=yes" 2>&1 >/tmp/.curlmsg$$
