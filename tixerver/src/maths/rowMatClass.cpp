...
// --- PointYX, TraffRect, TraffPolygon ---
...
bool TraffPolygon::containsPoint(const PointYX &pt) const {
    ...
    int winding = 0;
    for (size_t i = 0; i < pointList.size(); ++i) isect_line(winding, pointList[i], pointList[(i + 1) % pointList.size()], pt);
    return (winding % 2 != 0);
}
...
void TraffPolygon::computeBoundingRect() {
    ...
    for (size_t i = 1; i < pointList.size(); ++i) {
        minx = std::min(minx, pointList[i].x); maxx = std::max(maxx, pointList[i].x);
        miny = std::min(miny, pointList[i].y); maxy = std::max(maxy, pointList[i].y);
    }
...
void RowMat::cleanup() {
    if (refcounter) {
        ...
        refcounter = nullptr; data = nullptr; rows = nullptr; hh = 0; ww = 0;
    }
}
...
RowMat::RowMat(const RowMat& other) : refcounter(other.refcounter), external_data(other.external_data), data(other.data), rows(other.rows), hh(other.hh), ww(other.ww) {
    if (refcounter) ++(*refcounter);
}

RowMat::RowMat(RowMat&& other) noexcept : refcounter(other.refcounter), external_data(other.external_data), data(other.data), rows(other.rows), hh(other.hh), ww(other.ww) {
    other.refcounter = nullptr; other.data = nullptr; other.rows = nullptr;
}

RowMat& RowMat::operator=(const RowMat& other) {
    if (this != &other) { cleanup(); refcounter = other.refcounter; external_data = other.external_data; data = other.data; rows = other.rows; hh = other.hh; ww = other.ww; if (refcounter) ++(*refcounter); }
    return *this;
}

RowMat& RowMat::operator=(RowMat&& other) noexcept {
    if (this != &other) { cleanup(); refcounter = other.refcounter; external_data = other.external_data; data = other.data; rows = other.rows; hh = other.hh; ww = other.ww; other.refcounter = nullptr; }
    return *this;
}

cv::Mat RowMat::toCvView() const { return empty() ? cv::Mat() : cv::Mat((int)hh, (int)ww, CV_8UC1, data); }
...
void RowMat::getMeanStdDev(_FloatType &mean, _FloatType &stdDev) const {
    ...
    for (size_t i = 0; i < total; ++i) {
        double v = data[i]; sum += v; sq_sum += v * v;
    }
    mean = sum / total;
...
Frame::Frame(Frame&& other) noexcept : RowMat(std::move(other)), t(other.t), rgb(other.rgb), yuv(other.yuv) {}

Frame& Frame::operator=(Frame&& other) noexcept {
    if (this != &other) { RowMat::operator=(std::move(other)); t = other.t; rgb = other.rgb; yuv = other.yuv; }
    return *this;
}

Frame Frame::clone() const {
    Frame out;
    static_cast<RowMat&>(out) = RowMat::clone();
    out.t = this->t; out.rgb = this->rgb; out.yuv = this->yuv;
    return out;
}
